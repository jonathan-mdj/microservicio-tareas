...
def test_task_creation_fixed():
    print("🧪 Probando creación de tareas en Task Service MongoDB (usuario Profesor)...")
    
    try:
        # Primero verificar que el usuario Profesor existe
        print("\n1️⃣ Verificando usuario Profesor...")
        mongo_db.connect()
        users_collection = mongo_db.get_collection('users')
        user = users_collection.find_one({"username": "Profesor"})
        if user:
            print(f"   ✅ Usuario Profesor encontrado")
            print(f"   ID: {user['_id']}")
            print(f"   Role: {user['role']}")
        else:
            print(f"   ❌ Usuario Profesor no encontrado")
            return
        
        # Probar creación de tarea nueva
        print("\n2️⃣ Probando creación de tarea nueva...")
